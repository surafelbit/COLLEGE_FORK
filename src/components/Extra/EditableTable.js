...
const EditableTable = ({ initialData }) => {
    ...
    const columns = [
        {
            ...
            dataIndex: "photo",
            width: "20%",
            render: (text) => text ? (_jsx("img", { onClick: (e) => {
                    e.stopPropagation();
                    openModal(_jsx(ImageModal, { imageSrc: text }));
                }, src: text, alt: "student", className: "w-16 h-16 object-cover  border border-gray-300 dark:border-gray-600" })) : (_jsx("div", { className: "w-16 h-16 bg-gray-300 dark:bg-gray-700 flex items-center justify-center  text-gray-500 dark:text-gray-300 text-sm", children: "No Photo" })),
        },
        {
        ...
        },
        {
            ...
            dataIndex: "year",
            width: "12%",
            render: (_, record) => `${record.year || '-'} / ${record.semester ?? '-'}`,
        },
    ...
    _jsx(Table, { dataSource: initialData, columns: columns, rowKey: (record, index) => `${record.key}-${index}`, pagination: {
            ...pagination,
